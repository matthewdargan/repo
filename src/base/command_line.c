...
internal cmd_line_opt *cmd_line_insert_opt(arena *a, cmd_line *cmd_line, string8 string, string8list values) {
    ...
    if (existing_var != 0) {
        ...
        var->string = push_str8_copy(a, string);
...
internal cmd_line cmd_line_from_string_list(arena *a, string8list command_line) {
    cmd_line parsed = {{0}};
        ...
        if (is_option) {
            ...
            if (has_arguments) {
                for (string8node *n = node; n != 0; n = n->next) {
                    ...
                    u8 splits[] = {','};
                    string8list args_in_this_string = str8_split(a, string, splits, ARRAY_COUNT(splits), 0);
                    for (string8node *sub_arg = args_in_this_string.first; sub_arg != 0; sub_arg = sub_arg->next) {
                        str8_list_push(a, &arguments, sub_arg->string);
                    }
            ...
            cmd_line_insert_opt(a, &parsed, option_name, arguments);
        }
