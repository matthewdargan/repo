...
internal u64
str8_find_needle(string8 s, u64 start_pos, string8 needle, string_match_flags flags)
{
	u8 *p = s.str + start_pos;
	u64 stop_offset = MAX(s.size + 1, needle.size) - needle.size;
	u8 *stop_p = s.str + stop_offset;
	if (needle.size > 0) {
		u8 *string_opl = s.str + s.size;
...
internal u64
str8_find_needle_reverse(string8 s, u64 start_pos, string8 needle, string_match_flags flags)
{
	u64 result = 0;
	for (s64 i = s.size - start_pos - needle.size; i >= 0; --i) {
		string8 haystack = str8_substr(s, rng_1u64(i, i + needle.size));
...
internal string8
str8_substr(string8 s, rng1u64 range)
{
	range.min = MIN(range.min, s.size);
	range.max = MIN(range.max, s.size);
	s.str += range.min;
	s.size = dim_1u64(range);
	return s;
}
...
internal b32
str8_is_integer(string8 s, u32 radix)
{
	b32 result = 0;
	if (s.size > 0) {
		if (1 < radix && radix <= 16) {
...
internal u64
u64_from_str8(string8 s, u32 radix)
{
	u64 x = 0;
	if (1 < radix && radix <= 16) {
		for (u64 i = 0; i < s.size; ++i) {
			x *= radix;
			x += integer_symbol_reverse[s.str[i] & 0x7F];
		}
	}
	return x;
}
...
internal b32
try_u64_from_str8_c_rules(string8 s, u64 *x)
{
	b32 is_integer = 0;
	if (str8_is_integer(s, 10)) {
		is_integer = 1;
		*x = u64_from_str8(s, 10);
	} else {
		string8 hex_string = str8_skip(s, 2);
		if (str8_match(str8_prefix(s, 2), str8_lit("0x"), 0) && str8_is_integer(hex_string, 0x10)) {
			is_integer = 1;
			*x = u64_from_str8(hex_string, 0x10);
		} else if (str8_match(str8_prefix(s, 2), str8_lit("0b"), 0) && str8_is_integer(hex_string, 2)) {
			is_integer = 1;
			*x = u64_from_str8(hex_string, 2);
		} else {
			string8 oct_string = str8_skip(s, 1);
			if (str8_match(str8_prefix(s, 1), str8_lit("0"), 0) && str8_is_integer(hex_string, 010)) {
				is_integer = 1;
...
internal string8
str8_from_u64(arena *a, u64 value, u32 radix, u8 min_digits, u8 digit_group_separator)
{
	string8 result = {0};
	{
		string8 prefix = {0};
...
str8_list_push_node_set_string(string8list *list, string8node *node, string8 s)
{
...
str8_list_push(arena *a, string8list *list, string8 s)
{
	...
	str8_list_push_node_set_string(list, node, s);
	return node;
}

internal string8list
str8_split(arena *a, string8 s, u8 *split_chars, u64 split_char_count, string_split_flags flags)
{
	string8list list = {0};
	...
	for (; ptr < opl;) {
		...
		string8 s = str8_range(first, ptr);
		if (keep_empties || s.size > 0) {
...
internal string8
str8_list_join(arena *a, string8list *list, string_join *optional_params)
{
	string_join join = {{0}};
...
str8_array_from_list(arena *a, string8list *list)
{
	string8array array;
...
internal string8array
str8_array_reserve(arena *a, u64 count)
{
	string8array array;
	array.count = 0;
