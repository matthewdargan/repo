...
internal u64
get_total_pages(htmlDocPtr doc, xmlXPathCompExprPtr pagination_expr)
{
...
internal void
run_editor(arena *a, string8 path)
{
...
internal torrent_array
process_selected_torrents(arena *a, string8 path)
{
...
internal void
download_torrents(torrent_array torrents)
{
	if (torrents.count == 0) {
		return;
	}
