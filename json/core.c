...
json_parse_number(JSON_Parser *p)
{
  ...
  {
    u8 c = *p->ptr;
    if((c >= '0' && c <= '9') || c == '-' || c == '.' || c == 'e' || c == 'E' || c == '+') { p->ptr += 1; }
    else { break; }
  }
...
json_parse_object(JSON_Parser *p)
{
  ...
  if(p->ptr < p->end && *p->ptr == '}') { p->ptr += 1; return obj; }
...
json_parse_array(JSON_Parser *p)
{
  ...
  json_skip_whitespace(p);
  if(p->ptr < p->end && *p->ptr == ']') { p->ptr += 1; return arr; }
...
{
  json_skip_whitespace(p);
  if(p->ptr >= p->end) { return json_value_null(p->arena); }

  u8 c = *p->ptr;

  if(c == '"')                                      { return json_parse_string(p); }
  else if(c == '{')                                 { return json_parse_object(p); }
  else if(c == '[')                                 { return json_parse_array(p); }
  else if(json_match_literal(p, str8_lit("true")))  { return json_value_from_bool(p->arena, 1); }
  else if(json_match_literal(p, str8_lit("false"))) { return json_value_from_bool(p->arena, 0); }
  else if(json_match_literal(p, str8_lit("null")))  { return json_value_null(p->arena); }
  else if((c >= '0' && c <= '9') || c == '-')       { return json_parse_number(p); }
...
json_object_add(Arena *arena, JSON_Value *obj, String8 name, JSON_Value *value)
{
  ...
  if(obj->last != 0) { obj->last->next = member; }
  else               { obj->first = member; }
...
{
  if(obj->kind != JSON_ValueKind_Object) { return 0; }

  for(JSON_Member *member = obj->first; member != 0; member = member->next)
  {
    if(str8_match(member->name, name, 0)) { return member->value; }
  }
...
json_serialize_value(Arena *arena, String8List *list, JSON_Value *value)
{
  switch(value->kind)
  {
  case JSON_ValueKind_Null:  { str8_list_push(arena, list, str8_lit("null")); }break;
  case JSON_ValueKind_Bool:  { str8_list_push(arena, list, value->boolean ? str8_lit("true") : str8_lit("false")); }break;
  case JSON_ValueKind_Number:
  {
    String8 num_str = str8f(arena, "%g", value->number);
    str8_list_push(arena, list, num_str);
  }break;
  case JSON_ValueKind_String: { json_serialize_string(arena, list, value->string); }break;
  ...
  case JSON_ValueKind_Object:
  {
    ...
    {
      if(!first) { str8_list_push(arena, list, str8_lit(",")); }
      first = 0;
  ...
  case JSON_ValueKind_Array:
  {
    ...
    {
      if(i > 0) { str8_list_push(arena, list, str8_lit(",")); }
