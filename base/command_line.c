...
cmd_line_insert_opt(Arena *arena, CmdLine *cmd_line, String8 string, String8List values)
{
  ...
  if(existing_var != 0) { return existing_var; }

  CmdLineOpt *var    = push_array(arena, CmdLineOpt, 1);
  var->hash_next     = *slot;
...
cmd_line_from_string_list(Arena *arena, String8List arguments)
{
  ...
  for(String8Node *node = arguments.first->next, *next = 0; node != 0; node = next)
  {
    ...
    if(!after_passthrough_option)
    {
      ...
      else if(str8_match(str8_prefix(node->string, 1), str8_lit("-"), 0)) { option_name = str8_skip(option_name, 1); }
      else { is_option = 0; }
    }
    ...
    if(is_option)
    {
      ...
      b32 has_values = (value_signifier_position < option_name.size);
      ...
      if(has_values)
      {
        for(String8Node *n = node; n; n = n->next)
        {
          ...
          if(n == node) { string = value_portion_this_string; }
