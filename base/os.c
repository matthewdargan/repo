...
os_dense_time_from_timespec(timespec in)
{
  ...
  u64 result         = dense_time_from_date_time(date_time);
  return result;
}
...
os_file_properties_from_stat(struct stat *s)
{
  ...
  if(s->st_mode & S_IFDIR) { props.flags |= OS_FilePropertyFlag_Directory; }
  return props;
}
...
internal OS_SystemInfo *os_get_system_info(void) { return &os_system_info; }
...
os_file_open(OS_AccessFlags flags, String8 path)
{
  ...
  int lnx_flags = 0;
  if(flags & OS_AccessFlag_Read && flags & OS_AccessFlag_Write) { lnx_flags  = O_RDWR; }
...
{
  if(os_handle_match(file, os_handle_zero())) { return (OS_FileProperties){0}; }

  int fd                  = (int)file.u64[0];
  struct stat fd_stat     = {0};
  int fstat_result        = fstat(fd, &fd_stat);
  OS_FileProperties props = {0};
  if(fstat_result != -1) { props = os_file_properties_from_stat(&fd_stat); }
  return props;
}
...
os_delete_file_at_path(String8 path)
{
  Temp scratch      = scratch_begin(0, 0);
  String8 path_copy = str8_copy(scratch.arena, path);
  b32 result        = (remove((char *)path_copy.str) != -1);
  scratch_end(scratch);
  return result;
}
...
os_properties_from_file_path(String8 path)
{
  ...
  OS_FileProperties props = {0};
  if(stat_result != -1) { props = os_file_properties_from_stat(&f_stat); }
  scratch_end(scratch);
  return props;
}
...
os_make_directory(String8 path)
{
  Temp scratch      = scratch_begin(0, 0);
  String8 path_copy = str8_copy(scratch.arena, path);
  b32 result        = (mkdir((char *)path_copy.str, 0755) != -1);
  scratch_end(scratch);
  return result;
}
...
os_socket_connect_tcp(String8 host, u16 port)
{
  ...
  {
    if(host.size >= sizeof host_buffer) { return os_handle_zero(); }
  ...
  for(struct addrinfo *address_info = addrinfo_result; address_info != 0; address_info = address_info->ai_next)
  {
    ...
    {
    case AF_INET:  { ((struct sockaddr_in *)address_info->ai_addr)->sin_port = htons(port); }break;
    case AF_INET6: { ((struct sockaddr_in6 *)address_info->ai_addr)->sin6_port = htons(port); }break;
    default:       { continue; }
    }
    ...
    if(connect(fd, address_info->ai_addr, address_info->ai_addrlen) == 0) { break; }
    close(fd);
    fd = -1;
  }
  freeaddrinfo(addrinfo_result);

  if(fd < 0) { return os_handle_zero(); }
  OS_Handle handle = os_handle_zero();
  handle.u64[0] = fd;
  return handle;
}
...
os_socket_listen_tcp(u16 port)
{
  ...
  struct addrinfo *addrinfo_result = 0;
  if(getaddrinfo(0, port_buffer, &hints, &addrinfo_result) != 0) { return os_handle_zero(); }
...
{
  if(os_handle_match(listen_socket, os_handle_zero())) { return os_handle_zero(); }
  int listen_fd     = (int)listen_socket.u64[0];
  int connection_fd = accept(listen_fd, 0, 0);
  if(connection_fd < 0) { return os_handle_zero(); }
  int option = 1;
...
internal void os_sleep_milliseconds(u32 msec) { usleep(msec * Thousand(1)); }
