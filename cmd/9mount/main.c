...
reconnect_attempt_locked(Arena *arena)
{
  ...
  if(now - g_mount->last_reconnect_time < backoff)
  {
    return 0;
  }
...
{
  if(ins_atomic_u32_eval(&g_conn_state) == ConnState_Connected)
  {
    return 1;
  }
  b32 result = 0;
  MutexScope(g_reconnect_mutex)
  {
    result = reconnect_attempt_locked(arena);
  }
  return result;
}
...
walk_path(Arena *arena, String8 path)
{
  ...
  if(path.size == 0 || (path.size == 1 && path.str[0] == '/'))
  {
    return client->root;
  }
  ...
  if(trimmed.str[0] == '/')
  {
    trimmed = str8_skip(trimmed, 1);
  }

  ClientFid9P *result = 0;
  MutexScope(g_client_rpc_mutex)
  {
    result = client9p_fid_walk(arena, client->root, trimmed);
  }
  return result;
}
...
fs_getattr(const char *path, struct stat *stbuf, struct fuse_file_info *fi)
{
  ...
  if(fid != 0)
  {
    ...
    MutexScope(g_client_rpc_mutex)
    {
      dir = client9p_fid_stat(arena, fid);
    }
    if(dir.name.size == 0)
    {
      ins_atomic_u32_eval_assign(&g_conn_state, ConnState_Disconnected);
           ...
           enum fuse_readdir_flags flags)
{
  ...
  if(fid != 0)
  {
    MutexScope(g_client_rpc_mutex)
    {
      list = client9p_fid_read_dirs(arena, fid);
    }
  }
  else
  {
...
fs_open(const char *path, struct fuse_file_info *fi)
{
  ...
  else
  {
    ...
    MutexScope(g_client_rpc_mutex)
    {
      ...
      if(open_fid != 0)
      {
        ok = client9p_fid_open(arena, open_fid, mode);
      }
    }
    if(open_fid == 0)
    {
        ...
        struct fuse_file_info *fi)
{
  ...
  else
  {
    ...
    MutexScope(g_client_rpc_mutex)
    {
      n = client9p_fid_pread(arena, fid, buf, size, offset);
    }
    if(n < 0)
    {
      ins_atomic_u32_eval_assign(&g_conn_state, ConnState_Disconnected);
         ...
         struct fuse_file_info *fi)
{
  ...
  else
  {
    ...
    MutexScope(g_client_rpc_mutex)
    {
      n = client9p_fid_pwrite(arena, fid, (void *)buf, size, offset);
    }
    if(n < 0)
    {
      ins_atomic_u32_eval_assign(&g_conn_state, ConnState_Disconnected);
...
fs_create(const char *path, mode_t mode, struct fuse_file_info *fi)
{
  ...
  else
  {
    ...
    MutexScope(g_client_rpc_mutex)
    {
      ...
      if(new_fid != 0)
      {
        ok = client9p_fid_create(arena, new_fid, name, open_mode, mode);
      }
    }
    if(new_fid == 0)
    {
...
fs_unlink(const char *path)
{
  ...
  else
  {
    ...
    MutexScope(g_client_rpc_mutex)
    {
      ok = client9p_fid_remove(arena, fid);
    }
    result = ok ? 0 : -EIO;
  }
...
fs_rename(const char *from, const char *to, unsigned int flags)
{
  ...
  if(!str8_match(from_dir, to_dir, 0))
  {
    result = -EXDEV;
  }
  else
  {
    ...
    else
    {
      ...
      if(dst_fid != 0)
      {
        ...
        MutexScope(g_client_rpc_mutex)
        {
          ok = client9p_fid_remove(arena, dst_fid);
        }
        if(!ok)
        {
          result = -EIO;
      ...
      if(result == 0)
      {
        ...
        MutexScope(g_client_rpc_mutex)
        {
          ok = client9p_fid_wstat(arena, fid, dir);
        }
        result = ok ? 0 : -EIO;
      }
    }
...
fs_opendir(const char *path, struct fuse_file_info *fi)
{
  ...
  else
  {
    ...
    MutexScope(g_client_rpc_mutex)
    {
      ...
      if(fid != 0)
      {
        ok = client9p_fid_open(arena, fid, P9_OpenFlag_Read);
      }
    }
    if(fid == 0)
    {
