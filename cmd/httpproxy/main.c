...
h2_proxy_to_backend(H2_Session *session, H2_Stream *stream, HTTP_Request *req, String8 backend_host, u16 backend_port,
                    String8 path_prefix)
{
	log_infof("httpproxy: HTTP/2 proxying to backend %S:%u\n", backend_host, backend_port);
	...
	http_header_add(scratch.arena, &modified_req.headers, str8_lit("Connection"), str8_lit("close"));
	...
	while(remaining.size > 0)
	{
		...
		String8 header_line = str8_prefix(remaining, line_end);
		...
		if(colon < header_line.size)
		{
			...
			if(str8_match(name, str8_lit("Location"), StringMatchFlag_CaseInsensitive))
			{
				if(value.size > 0 && value.str[0] == '/')
				{
					...
					log_infof("httpproxy: rewrote Location header to %S\n", value);
				}
	...
	h2_send_response(session, stream, res);
	log_infof("httpproxy: proxied %llu bytes from backend via HTTP/2\n", response_size);
	scratch_end(scratch);
}
...
handle_h2_request(H2_Session *session, H2_Stream *stream, HTTP_Request *req, void *user_data)
{
	...
	log_infof("httpproxy: handle_h2_request called for path=%S\n", req->path);
	...
	if(str8_find_needle(req->path, 0, str8_lit("/jellyfin"), 0) == 0)
	{
		...
		log_info(str8_lit("httpproxy: proxying to Jellyfin backend (nas:8096) via HTTP/2\n"));
...
handle_h2_connection(SSL *ssl, OS_Handle connection_socket, Arena *connection_arena)
{
	...
	H2_Session *session =
	    h2_session_alloc(connection_arena, ssl, connection_socket, proxy_ctx->workers, handle_h2_request, proxy_ctx);
