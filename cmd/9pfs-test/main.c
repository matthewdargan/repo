...
test_tmp_nested_files(Arena *arena, Client9P *client)
{
	...
	if(!test_create_file(arena, client, str8_lit("tmp/nested/file1")))
	{
		return 0;
	}
	if(!test_create_file(arena, client, str8_lit("tmp/nested/file2")))
	{
		return 0;
	}
	Dir9P dir = client9p_stat(arena, client, str8_lit("tmp/nested/file1"));
	return dir.name.size > 0;
}
...
test_tmp_stat(Arena *arena, Client9P *client)
{
	...
	if(!test_write_read(arena, client, str8_lit("tmp/stat_test"), test_data))
	{
		return 0;
	}
...
test_tmp_wstat_truncate(Arena *arena, Client9P *client)
{
	...
	if(!test_write_read(arena, client, str8_lit("tmp/truncate_test"), test_data))
	{
		return 0;
	}
...
test_tmp_wstat_chmod(Arena *arena, Client9P *client)
{
	...
	b32 result = 1;
	result = result && test_wstat_chmod(arena, client, str8_lit("tmp/chmod_test"), 0755);
	result = result && test_wstat_chmod(arena, client, str8_lit("tmp/chmod_test"), 0644);
	return result;
}
...
test_tmp_wstat_rename(Arena *arena, Client9P *client)
{
	...
	Dir9P dir = client9p_stat(arena, client, str8_lit("tmp/rename_old"));
	if(dir.name.size == 0)
	{
		return 0;
	}
	dir.name = str8_lit("rename_new");
	if(!client9p_wstat(arena, client, str8_lit("tmp/rename_old"), dir))
	{
		return 0;
	}

	Dir9P renamed_dir = client9p_stat(arena, client, str8_lit("tmp/rename_new"));
...
test_tmp_large_file(Arena *arena, Client9P *client)
{
	...
	b32 result = test_write_read(arena, client, str8_lit("tmp/large_file"), large_str);
	scratch_end(scratch);
	return result;
}
...
{
	ClientFid9P *fid =
	    test_open_or_create(arena, client, str8_lit("tmp/partial_write"), P9_OpenFlag_Write | P9_OpenFlag_Truncate, 0666);
	if(fid == 0)
	{
		return 0;
	}
	...
	fid = client9p_open(arena, client, str8_lit("tmp/partial_write"), P9_OpenFlag_Read);
	if(fid == 0)
	{
		return 0;
	}
...
test_tmp_seek_read(Arena *arena, Client9P *client)
{
	...
	if(!test_write_read(arena, client, str8_lit("tmp/seek_file"), test_data))
	{
		return 0;
	}

	ClientFid9P *fid = client9p_open(arena, client, str8_lit("tmp/seek_file"), P9_OpenFlag_Read);
	if(fid == 0)
	{
		return 0;
	}
...
test_tmp_qid_consistency(Arena *arena, Client9P *client)
{
	...
	Dir9P dir1 = client9p_stat(arena, client, str8_lit("tmp/qid_test"));
	if(dir1.name.size == 0)
	{
		return 0;
	}

	Dir9P dir2 = client9p_stat(arena, client, str8_lit("tmp/qid_test"));
	if(dir2.name.size == 0)
	{
		return 0;
	}
...
run_tests(Arena *arena, String8 address)
{
	...
	if(test_tmp_create_file(arena, client))
	{
		log_info(str8_lit("PASS: tmp_create_file\n"));
		passed += 1;
	}
	else
	{
		log_error(str8_lit("FAIL: tmp_create_file\n"));
		failed += 1;
	}

	if(test_tmp_write_read(arena, client))
	{
		log_info(str8_lit("PASS: tmp_write_read\n"));
		passed += 1;
	}
	else
	{
		log_error(str8_lit("FAIL: tmp_write_read\n"));
		failed += 1;
	}

	if(test_tmp_create_directory(arena, client))
	{
		log_info(str8_lit("PASS: tmp_create_directory\n"));
		passed += 1;
	}
	else
	{
		log_error(str8_lit("FAIL: tmp_create_directory\n"));
		failed += 1;
	}

	if(test_tmp_nested_files(arena, client))
	{
		log_info(str8_lit("PASS: tmp_nested_files\n"));
		passed += 1;
	}
	else
	{
		log_error(str8_lit("FAIL: tmp_nested_files\n"));
		failed += 1;
	}

	if(test_tmp_readdir(arena, client))
	{
		log_info(str8_lit("PASS: tmp_readdir\n"));
		passed += 1;
	}
	else
	{
		log_error(str8_lit("FAIL: tmp_readdir\n"));
		failed += 1;
	}

	if(test_tmp_stat(arena, client))
	{
		log_info(str8_lit("PASS: tmp_stat\n"));
		passed += 1;
	}
	else
	{
		log_error(str8_lit("FAIL: tmp_stat\n"));
		failed += 1;
	}

	if(test_tmp_wstat_truncate(arena, client))
	{
		log_info(str8_lit("PASS: tmp_wstat_truncate\n"));
		passed += 1;
	}
	else
	{
		log_error(str8_lit("FAIL: tmp_wstat_truncate\n"));
		failed += 1;
	}

	if(test_tmp_wstat_chmod(arena, client))
	{
		log_info(str8_lit("PASS: tmp_wstat_chmod\n"));
		passed += 1;
	}
	else
	{
		log_error(str8_lit("FAIL: tmp_wstat_chmod\n"));
		failed += 1;
	}

	if(test_tmp_wstat_rename(arena, client))
	{
		log_info(str8_lit("PASS: tmp_wstat_rename\n"));
		passed += 1;
	}
	else
	{
		log_error(str8_lit("FAIL: tmp_wstat_rename\n"));
		failed += 1;
	}

	if(test_tmp_remove_file(arena, client))
	{
		log_info(str8_lit("PASS: tmp_remove_file\n"));
		passed += 1;
	}
	else
	{
		log_error(str8_lit("FAIL: tmp_remove_file\n"));
		failed += 1;
	}

	if(test_tmp_remove_directory(arena, client))
	{
		log_info(str8_lit("PASS: tmp_remove_directory\n"));
		passed += 1;
	}
	else
	{
		log_error(str8_lit("FAIL: tmp_remove_directory\n"));
		failed += 1;
	}

	if(test_tmp_large_file(arena, client))
	{
		log_info(str8_lit("PASS: tmp_large_file\n"));
		passed += 1;
	}
	else
	{
		log_error(str8_lit("FAIL: tmp_large_file\n"));
		failed += 1;
	}

	if(test_tmp_partial_write(arena, client))
	{
		log_info(str8_lit("PASS: tmp_partial_write\n"));
		passed += 1;
	}
	else
	{
		log_error(str8_lit("FAIL: tmp_partial_write\n"));
		failed += 1;
	}

	if(test_tmp_seek_read(arena, client))
	{
		log_info(str8_lit("PASS: tmp_seek_read\n"));
		passed += 1;
	}
	else
	{
		log_error(str8_lit("FAIL: tmp_seek_read\n"));
		failed += 1;
	}

	if(test_tmp_qid_consistency(arena, client))
	{
		log_info(str8_lit("PASS: tmp_qid_consistency\n"));
		passed += 1;
	}
	else
	{
		log_error(str8_lit("FAIL: tmp_qid_consistency\n"));
		failed += 1;
	}
