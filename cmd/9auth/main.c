...
fid_aux_alloc(Server9P *server)
{
  ...
  if(aux != 0) { server->fid_aux_free_list = (FidAuxiliary9P *)aux->next; }
  else         { aux = push_array_no_zero(server->arena, Auth_FidAuxiliary9P, 1); }
...
internal void srv_auth(ServerRequest9P *request) { server9p_respond(request, str8_lit("authentication not required")); }
...
srv_walk(ServerRequest9P *request)
{
  ...
  for(u64 i = 0; i < request->in_msg.walk_name_count; i += 1)
  {
    ...
    {
      if(i == 0) { request->out_msg.walk_qids[i] = request->fid->qid; }
      else       { request->out_msg.walk_qids[i] = request->out_msg.walk_qids[i - 1]; }
      continue;
    }
...
internal void srv_create(ServerRequest9P *request) { server9p_respond(request, str8_lit("cannot create files")); }
...
srv_read(ServerRequest9P *request)
{
  ...
  {
    String8 data                  = auth_fs_read(request->scratch.arena, auth_fs_state, aux->file_type, aux->conv, request->in_msg.file_offset, request->in_msg.byte_count);
...
{
  Auth_FidAuxiliary9P *aux = fid_aux_get(request->server, request->fid);
  b32 success = auth_fs_write(request->scratch.arena, auth_fs_state, aux->file_type, &aux->conv, request->in_msg.payload_data);
  ...
  else { server9p_respond(request, str8_lit("write failed")); }
}
...
srv_stat(ServerRequest9P *request)
{
  ...
  switch(aux->file_type)
  {
  case Auth_File_None: {}break;
  case Auth_File_Root: { info = auth_fs_stat_root(auth_fs_state); }break;
  case Auth_File_RPC:  { info = auth_fs_lookup(auth_fs_state, str8_lit("rpc")); }break;
  case Auth_File_Ctl:  { info = auth_fs_lookup(auth_fs_state, str8_lit("ctl")); }break;
  case Auth_File_Log:  { info = auth_fs_lookup(auth_fs_state, str8_lit("log")); }break;
  }
...
handle_connection(OS_Handle connection_socket)
{
  ...
  else { log_info(str8_lit("9auth: connection established (peer credentials unavailable)\n")); }
  ...
  for(;;)
  {
    ...
    case Msg9P_Tflush:   { srv_flush(request); }break;
...
entry_point(CmdLine *cmd_line)
{
  ...
  if(socket_path.size == 0) { socket_path = str8_lit("/run/9auth/socket"); }
  ...
  else if(export_credential)
  {
    ...
    for(u64 i = 0; i < ring.count; i += 1)
    {
      ...
      if(str8_match(key->user, user, 0) && str8_match(key->auth_id, auth_id, 0))
      {
        ...
        if(export_key.type == Auth_Proto_Ed25519) { MemoryZero(export_key.ed25519_private_key, sizeof(export_key.ed25519_private_key)); }
        String8 add_error = str8_zero();
