...
//
// These tests require real FIDO2 hardware (Yubikey) and test the complete
// authentication flow:
//   1. Register credential with 9auth
//   2. Use client9p_auth() to authenticate to 9pfs server
//   3. Verify authenticated mount works
//
...
entry_point(CmdLine *cmd_line)
{
  ...
  if(cmd_line->inputs.node_count < 2)
  {
    ...
    fprintf(stderr, "\n");
    fprintf(stderr, "End-to-end integration test requiring FIDO2 hardware (Yubikey).\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "Example:\n");
    fprintf(stderr, "  9auth-integration-test unix!/var/run/9auth unix!/tmp/9pfs.sock\n");
    fprintf(stderr, "\n");
    fflush(stderr);
    return;
  }
  ...
  fprintf(stdout, "Touch your Yubikey when it blinks during each test\n");
  fprintf(stdout, "\n");
  fflush(stdout);
