...
auth_ed25519_register_credential(Arena *arena, Auth_Ed25519_RegisterParams params, Auth_Key *out_key, String8 *out_error)
{
  ...
  SecureMemoryZero(public_key, 32);
  SecureMemoryZero(private_key, 32);
...
{
  if(params == 0 || signature == 0 || out_error == 0) { return 0; }

  b32 success       = 0;
  EVP_MD_CTX *mdctx = 0;
  EVP_PKEY *pkey    = 0;

  SecureMemoryZero(signature, 64);
  ...
  if(EVP_DigestSignInit(mdctx, 0, 0, 0, pkey) <= 0)
  {
...
cleanup:
  ...
  if(pkey != 0)  { EVP_PKEY_free(pkey); }
  if(!success)   { SecureMemoryZero(signature, 64); }
  return success;
}
...
{
  if(params == 0 || out_error == 0) { return 0; }
  ...
  if(EVP_DigestVerifyInit(mdctx, 0, 0, 0, pkey) <= 0)
  {
