...
auth_ed25519_register_credential(Arena *arena, Auth_Ed25519_RegisterParams params, Auth_Key *out_key, String8 *out_error)
{
  ...
  if(ctx == 0)                              { *out_error = str8_lit("ed25519: failed to create key generation context"); }
  else if(EVP_PKEY_keygen_init(ctx) <= 0)   { *out_error = str8_lit("ed25519: failed to initialize key generation"); }
  else if(EVP_PKEY_keygen(ctx, &pkey) <= 0) { *out_error = str8_lit("ed25519: failed to generate keypair"); }
  else
  {
    u8 public_key[32];
    u8 private_key[32];
...
internal b32
auth_ed25519_sign_challenge(Auth_Ed25519_SignParams *params, u8 signature[64], String8 *out_error)
{
  EVP_MD_CTX *mdctx = 0;
  EVP_PKEY *pkey    = 0;
  b32 success       = 0;
  ...
  if(pkey == 0) { *out_error = str8_lit("ed25519: failed to create private key"); }
  else
  {
    mdctx = EVP_MD_CTX_new();
    if(mdctx == 0)                                         { *out_error = str8_lit("ed25519: failed to create signing context"); }
    else if(EVP_DigestSignInit(mdctx, 0, 0, 0, pkey) <= 0) { *out_error = str8_lit("ed25519: failed to initialize signing"); }
    else
    {
  ...
  if(mdctx != 0) { EVP_MD_CTX_free(mdctx); }
  if(pkey != 0)  { EVP_PKEY_free(pkey); }
  return success;
}
...
auth_ed25519_verify_signature(Auth_Ed25519_VerifyParams *params, String8 *out_error)
{
  ...
  if(pkey == 0) { *out_error = str8_lit("ed25519: failed to create public key"); }
  else
  {
    mdctx = EVP_MD_CTX_new();
    if(mdctx == 0)                                           { *out_error = str8_lit("ed25519: failed to create verification context"); }
    else if(EVP_DigestVerifyInit(mdctx, 0, 0, 0, pkey) <= 0) { *out_error = str8_lit("ed25519: failed to initialize verification"); }
    else
    {
      int verify_result = EVP_DigestVerify(mdctx, params->signature, 64, params->challenge, 32);
      if(verify_result <= 0) { *out_error = str8_lit("ed25519: signature verification failed"); }
      else                   { success = 1; }
    }
  }
