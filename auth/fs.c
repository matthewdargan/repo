...
auth_fs_lookup(Auth_FS_State *fs, String8 path)
{
  ...
  String8 name = path;
  if(name.str[0] == '/') { name = str8_skip(name, 1); }
  ...
  else if(str8_match(name, str8_lit("log"), 0))
  {
    ...
    info.qid_path = 3;
    info.mode     = 0400;
    MutexScope(fs->mutex) { info.size = fs->log_size; }
  }

  return info;
}

internal Auth_File_Info auth_fs_stat_root(Auth_FS_State *fs) { return auth_fs_lookup(fs, str8_lit("/")); }
...
auth_fs_read(Arena *arena, Auth_FS_State *fs, Auth_File_Type file_type, Auth_Conv *conv, u64 offset, u64 count)
{
  ...
  case Auth_File_RPC:
  {
    ...
    else if(response.error.size > 0) { result = response.error; }
  }break;

  case Auth_File_Log:
  {
    String8 log_content = str8_zero();
    MutexScope(fs->mutex) { log_content = str8_list_join(arena, fs->log_entries, 0); }
...
auth_fs_write(Arena *arena, Auth_FS_State *fs, Auth_File_Type file_type, Auth_Conv **conv, String8 data)
{
  ...
  case Auth_File_Ctl:
  {
    ...
    if(str8_match(command, str8_lit("register"), 0))
    {
      ...
      String8 credential_id_hex = str8_zero();
      String8 pubkey_hex        = str8_zero();

      for(String8Node *node = parts.first->next; node != 0; node = node->next)
      {
        String8List kv = str8_split(arena, node->string, (u8 *)"=", 1, 0);
        if(kv.node_count != 2) { continue; }

        String8 key   = kv.first->string;
        String8 value = kv.first->next->string;

        if(str8_match(key, str8_lit("user"), 0))               { user              = value; }
        else if(str8_match(key, str8_lit("auth-id"), 0))       { auth_id           = value; }
        else if(str8_match(key, str8_lit("proto"), 0))         { proto             = value; }
        else if(str8_match(key, str8_lit("credential-id"), 0)) { credential_id_hex = value; }
        else if(str8_match(key, str8_lit("pubkey"), 0))        { pubkey_hex        = value; }
      }

      if(user.size == 0 || auth_id.size == 0 || proto.size == 0)
      {
        auth_fs_log(fs, str8_lit("register_failed: missing required parameters\n"));
        break;
      }
      ...
      {
        if(credential_id_hex.size > 0 && pubkey_hex.size > 0)
        {
          String8 cred_bytes = bytes_from_hex(arena, credential_id_hex);
          String8 pub_bytes  = bytes_from_hex(arena, pubkey_hex);
      ...
      {
        auth_fs_log(fs, str8f(arena, "register_failed: unsupported proto=%S\n", proto));
        break;
      }
