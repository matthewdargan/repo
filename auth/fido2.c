...
auth_fido2_enumerate_devices(Arena *arena)
{
  ...
  for(size_t i = 0; i < dev_count; i += 1)
  {
    ...
    if(manufacturer != 0) { info->manufacturer = str8_copy(arena, str8_cstring((char *)manufacturer)); }
...
auth_fido2_register_credential(Arena *arena, Auth_Fido2_RegisterParams params, Auth_Key *out_key, String8 *out_error)
{
  ...
  dev = fido_dev_new();
  if(dev == 0) { *out_error = str8_lit("fido2: failed to allocate device"); goto cleanup; }
  ...
  if(r != FIDO_OK) { *out_error = str8f(arena, "fido2: failed to open device: %S", auth_fido2_error_string(arena, r)); goto cleanup; }

  cred = fido_cred_new();
  if(cred == 0) { *out_error = str8_lit("fido2: failed to allocate credential"); goto cleanup; }

  r = fido_cred_set_type(cred, COSE_ES256);
  if(r != FIDO_OK) { *out_error = str8f(arena, "fido2: failed to set credential type: %S", auth_fido2_error_string(arena, r)); goto cleanup; }
  ...
  if(r != FIDO_OK) { *out_error = str8f(arena, "fido2: failed to set relying party: %S", auth_fido2_error_string(arena, r)); goto cleanup; }
  ...
  if(r != FIDO_OK) { *out_error = str8f(arena, "fido2: failed to set client data hash: %S", auth_fido2_error_string(arena, r)); goto cleanup; }

  if(params.require_uv)
  {
    r = fido_cred_set_uv(cred, FIDO_OPT_TRUE);
    if(r != FIDO_OK) { *out_error = str8f(arena, "fido2: failed to set UV option: %S", auth_fido2_error_string(arena, r)); goto cleanup; }
  }

  r = fido_cred_set_rk(cred, FIDO_OPT_FALSE);
  if(r != FIDO_OK) { *out_error = str8f(arena, "fido2: failed to set RK option: %S", auth_fido2_error_string(arena, r)); goto cleanup; }

  r = fido_dev_make_cred(dev, cred, 0);
  if(r != FIDO_OK) { *out_error = str8f(arena, "fido2: failed to make credential: %S", auth_fido2_error_string(arena, r)); goto cleanup; }
...
auth_fido2_get_assertion(Arena *arena, Auth_Fido2_AssertParams *params, Auth_Fido2_Assertion *out_assertion, String8 *out_error)
{
  ...
  if(assert == 0) { *out_error = str8_lit("fido2: failed to allocate assertion"); goto cleanup; }
  ...
  if(r != FIDO_OK) { *out_error = str8f(arena, "fido2: failed to allow credential: %S", auth_fido2_error_string(arena, r)); goto cleanup; }
  ...
  if(r != FIDO_OK) { *out_error = str8f(arena, "fido2: failed to get assertion: %S", auth_fido2_error_string(arena, r)); goto cleanup; }
