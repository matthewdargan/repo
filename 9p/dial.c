...
dial9p_resolve_port(String8 port, String8 protocol)
{
	...
	{
		return (u16)u64_from_str8(port, 10);
	}
...
dial9p_parse(Arena *arena, String8 dial_string, String8 default_protocol, String8 default_port)
{
	...
	if(default_protocol.size == 0)
	{
		default_protocol = str8_lit("tcp");
	}
	...
	if(bang_pos >= dial_string.size)
	{
		...
		{
			result.host = str8_copy(arena, dial_string);
			if(default_port.size > 0)
			{
				result.port = dial9p_resolve_port(default_port, default_protocol);
			}
			if(str8_match(default_protocol, str8_lit("tcp"), 0))
			{
				result.protocol = Dial9PProtocol_TCP;
			}
		}
	...
	else
	{
		...
		if(default_port.size > 0)
		{
			result.port = dial9p_resolve_port(default_port, protocol);
		}
	}

	if(str8_match(protocol, str8_lit("tcp"), 0))
	{
		result.protocol = Dial9PProtocol_TCP;
	}
	return result;
...
dial9p_connect(Arena *scratch, String8 dial_string, String8 default_protocol, String8 default_port)
{
	...
	if(address.protocol == Dial9PProtocol_Unix)
	{
		return os_socket_connect_unix(address.host);
	}
	else if(address.protocol == Dial9PProtocol_TCP)
	{
		return os_socket_connect_tcp(address.host, address.port);
	}
	return os_handle_zero();
}
...
dial9p_listen(String8 dial_string, String8 default_protocol, String8 default_port)
{
	...
	if(address.protocol == Dial9PProtocol_Unix)
	{
		result = os_socket_listen_unix(address.host);
	}
	else if(address.protocol == Dial9PProtocol_TCP)
	{
		result = os_socket_listen_tcp(address.port);
	}
	scratch_end(scratch);
	return result;
}
