{
  perSystem = {
    ...
    checks = {
      ...
      "9auth-integration-test" = let
        authAgent = self'.packages."9auth-debug";
        server = self'.packages."9pfs-debug";
        testClient = self'.packages."9auth-integration-test-debug";
      in
        pkgs.runCommand "9auth-integration-test-check" {
          ...
          ${server}/bin/9pfs --root="$testdir/fs-root" --require-auth --auth-server=unix!"$auth_socket" unix!"$fs_socket" &
          fs_pid=$!
          trap "kill $fs_pid $auth_pid 2>/dev/null || true; rm -rf $testdir" EXIT
          sleep 2

          ${testClient}/bin/9auth-integration-test unix!"$auth_socket" unix!"$fs_socket" > test_output.txt 2>&1 || true
          cat test_output.txt
