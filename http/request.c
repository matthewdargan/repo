...
http_request_parse(Arena *arena, String8 data)
{
  ...
  if(data.size == 0) { return req; }

  u64 line_end = str8_find_needle(data, 0, str8_lit("\r\n"), 0);
  if(line_end >= data.size) { return req; }
  ...
  if(parts.node_count < 3) { return req; }
  ...
  if(header_end >= remainder.size) { header_end = remainder.size; }
